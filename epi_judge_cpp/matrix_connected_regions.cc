...
void FlipColorHelper(int x, int y, vector<deque<bool>>& image, bool flipColor){
  if(x < 0 || x >= image.size() || y < 0 || y >= image[x].size() || image[x][y] == flipColor){
    return;
  }

  image[x][y] = flipColor;
  FlipColorHelper(x - 1, y, image, flipColor);
  FlipColorHelper(x + 1, y, image, flipColor);
  FlipColorHelper(x, y - 1, image, flipColor);
  FlipColorHelper(x, y + 1, image, flipColor);
...
void FlipColor(int x, int y, vector<deque<bool>>* image_ptr) {
  ...
  bool flipColor = !(image[x][y]);
  FlipColorHelper(x, y, image, flipColor);
}
