...
void DFS(GraphVertex* vertex, unordered_map<GraphVertex*, GraphVertex*>& map){
  ...
  for(GraphVertex* adjacentVertex : vertex->edges){
    DFS(adjacentVertex, map);
    newVertex->edges.push_back(map[adjacentVertex]);
  }
}

GraphVertex* CloneGraph(GraphVertex* graph) {
  unordered_map<GraphVertex*, GraphVertex*> map;
  DFS(graph, map);
  return map[graph];
}
